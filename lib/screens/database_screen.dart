...
class _DatabaseScreenState extends State<DatabaseScreen> {
                    ...
                    Row(
                      children: [
                        if (widget.papers.isNotEmpty) ...[
                          ...
                          const SizedBox(width: 12),
                          ElevatedButton.icon(
                            onPressed: _isAutomating ? null : _automateScreening,
                            icon: _isAutomating
                                ? const SizedBox(
                                    width: 14,
                                    height: 14,
  ...
  Future<void> _automateScreening() async {
    if (widget.selectedModel == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Please select an AI model in Settings first.')));
      }
      return;
    }

    final papersToScreen = widget.papers
        .where((p) => p.prismaStage == PrismaStage.identified || p.prismaStage == PrismaStage.screened)
        .toList();

    if (papersToScreen.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('No identified or screened papers to automate.')));
      }
      return;
    }
    ...
    int processed = 0;
    for (final p in papersToScreen) {
      if (!mounted) break;
      final prompt = '''
Task: Evaluate this research paper for PRISMA screening.
Topic: ${p.topic ?? 'General'}
Title: ${p.title}
Abstract: ${p.abstract_}

Determine if this should be 'included', 'eligible', or 'excluded'.
...
''';
      try {
        ...
        final req = await http.post(
          Uri.parse('${widget.ollamaIp}/api/generate'),
        ...
        if (req.statusCode == 200) {
          ...
          var resp = data['response'].toString().trim();
          int startIdx = resp.indexOf('{');
          int endIdx = resp.lastIndexOf('}');
          if (startIdx != -1 && endIdx != -1 && endIdx > startIdx) {
            resp = resp.substring(startIdx, endIdx + 1);
          }
          final jsonResp = jsonDecode(resp);
          final stageStr = jsonResp['stage']?.toString().toLowerCase();
          
          if (stageStr == 'included') {
            p.prismaStage = PrismaStage.included;
          } else if (stageStr == 'eligible') {
            p.prismaStage = PrismaStage.eligible;
          } else {
            p.prismaStage = PrismaStage.excluded;
            p.exclusionReason = jsonResp['reason']?.toString();
          }
          _updatePaper(p);
        }
      } catch (e) {
        print('Error automating screening: ${(e)}');
      }
      processed++;
      setState((){});
    }
    ...
    if (mounted) {
      setState(() => _isAutomating = false);
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Automated screening completed for $processed papers.')),
      );
    }
  }
  ...
  Future<void> _showCsvExportDialog() async {
    bool expTitle = true;
    bool expAuthors = true;
    bool expYear = true;
    bool expMethod = true;
    bool expSource = true;
    bool expAbstract = false;
    bool expStage = true;
    bool expTopic = true;
    ...
    await showDialog(
      ...
      builder: (ctx) {
        return StatefulBuilder(builder: (ctx, setDialogState) {
            return AlertDialog(
              backgroundColor: AppTheme.surface,
              title: Text('Export CSV', style: GoogleFonts.interTight(fontSize: 18, fontWeight: FontWeight.w600)),
              content: SingleChildScrollView(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Text('Select Papers:', style: AppTheme.labelSmall()),
                    CheckboxListTile(
                      title: const Text('Export current filtered/searched list only'),
                      value: onlyFiltered,
                      onChanged: (v) => setDialogState(() => onlyFiltered = v!),
                      controlAffinity: ListTileControlAffinity.leading,
                      contentPadding: EdgeInsets.zero,
                    ),
                    const SizedBox(height: 16),
                    Text('Select Columns to Export:', style: AppTheme.labelSmall()),
                    Wrap(
                      spacing: 12,
                      runSpacing: 0,
                      children: [
                        _exportCheck('Title', expTitle, (v) => setDialogState(() => expTitle = v)),
                        _exportCheck('Authors', expAuthors, (v) => setDialogState(() => expAuthors = v)),
                        _exportCheck('Year', expYear, (v) => setDialogState(() => expYear = v)),
                        _exportCheck('Methodology', expMethod, (v) => setDialogState(() => expMethod = v)),
                        _exportCheck('Source/Venue', expSource, (v) => setDialogState(() => expSource = v)),
                        _exportCheck('Abstract', expAbstract, (v) => setDialogState(() => expAbstract = v)),
                        _exportCheck('PRISMA Stage', expStage, (v) => setDialogState(() => expStage = v)),
                        _exportCheck('Topic', expTopic, (v) => setDialogState(() => expTopic = v)),
                      ],
                    ),
                  ],
                ),
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    _exportSelectiveCsv(
                      ...
                      expTitle: expTitle,
                      expAuthors: expAuthors,
                      expYear: expYear,
                      expMethod: expMethod,
                      expSource: expSource,
                      expAbstract: expAbstract,
                      expStage: expStage,
                      expTopic: expTopic,
                    );
                  },
  ...
  Widget _exportCheck(String label, bool value, ValueChanged<bool> onChanged) {
    return SizedBox(
      width: 150,
  ...
  Future<void> _exportSelectiveCsv({
    required bool onlyFiltered, required bool expTitle, required bool expAuthors, required bool expYear,
    required bool expMethod, required bool expSource, required bool expAbstract, required bool expStage, required bool expTopic
  }) async {
    try {
      final papersToExport = onlyFiltered ? _filteredPapers : widget.papers;
      if (papersToExport.isEmpty) return;

      final rows = [];
      final headers = [];
      if (expTitle) headers.add('Title');
      if (expAuthors) headers.add('Authors');
      if (expYear) headers.add('Year');
      if (expMethod) headers.add('Methodology');
      if (expSource) headers.add('Source');
      if (expAbstract) headers.add('Abstract');
      if (expStage) headers.add('PRISMA Stage');
      if (expTopic) headers.add('Topic');
      
      rows.add(headers.map((h) => '"$h"').join(','));

      for (final p in papersToExport) {
        final row = [];
        if (expTitle) row.add('"${p.title.replaceAll('"', '""')}"');
        if (expAuthors) row.add('"${p.authors.replaceAll('"', '""')}"');
        if (expYear) row.add('"${p.year ?? ''}"');
        if (expMethod) row.add('"${p.methodology.replaceAll('"', '""')}"');
        if (expSource) row.add('"${(p.source ?? '').replaceAll('"', '""')}"');
        if (expAbstract) row.add('"${p.abstract_.replaceAll('"', '""')}"');
        if (expStage) row.add('"${p.prismaStage.name}"');
        if (expTopic) row.add('"${(p.topic ?? '').replaceAll('"', '""')}"');
        
        rows.add(row.join(','));
      }

      final csvData = rows.join('\n');
      final String? outputFile = await FilePicker.platform.saveFile(
        dialogTitle: 'Save Database Export',
        fileName: 'Research_Database_Export.csv',
      ...
      if (outputFile != null) {
        ...
        if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Exported successfully to $outputFile'), backgroundColor: AppTheme.success));
        }
      }
    } catch(e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to export: $e'), backgroundColor: AppTheme.error));
      }
    }
  }
